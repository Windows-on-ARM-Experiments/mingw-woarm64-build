...
print_summary() {
    ...
    TOTAL_TESTS=$(($EXPECTED_PASSES + $UNEXPECTED_FAILURES + $UNEXPECTED_SUCCESSES + \
        $EXPECTED_FAILURES + $UNRESOLVED_TESTCASES + $UNSUPPORTED_TESTS + $DEJAGNU_ERRORS))
    RELIABILITY_RATE=$(($TOTAL_EXPECTED * 100 / $TOTAL_TESTS))
...
TOTAL_TESTS=$(($TOTAL_EXPECTED_PASSES + $TOTAL_UNEXPECTED_FAILURES + $TOTAL_UNEXPECTED_SUCCESSES + \
    $TOTAL_EXPECTED_FAILURES + $TOTAL_UNRESOLVED_TESTCASES + $TOTAL_UNSUPPORTED_TESTS))
